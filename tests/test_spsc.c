...
void test_lf_spsc() {
  ...
  test_stress_ops_count_run("Lock Free (LF) SPSC (buffer = 1)", 1, 1, 5, q_1);
  test_fifo_run("Lock Free (LF) SPSC (buffer = 1)", 5, q_1);
  queue_destroy(q_1);

  queue_t *q_16 = lf_spsc_create(16);
  test_stress_ops_count_run("Lock Free (LF) SPSC (buffer = 16)", 1, 1, 5, q_16);
  test_fifo_run("Lock Free (LF) SPSC (buffer = 16)", 5, q_16);
  queue_destroy(q_16);

  queue_t *q_1024_1024 = lf_spsc_create(1024 * 1024);
  test_stress_ops_count_run("Lock Free (LF) SPSC (buffer = 1024 * 1024)", 1, 1, 5, q_1024_1024);
